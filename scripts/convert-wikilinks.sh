...
# 说明:  原地修改 notes/ 下的 .md；内链一律写成绝对路径 /notes/xxx.html
#        （做不到的链接会退化成纯文本，并在日志里列出）
#
set -euo pipefail
...
find "$root" -type f -name '*.md' -print0 | while IFS= read -r -d '' file; do
  awk -v map_file="$map_file" -v prefix="/$root/" '
    ...
    END {
      ...
      strip_h1 = (!(NR > 0 && converted[1] == "---") && start > 0 && converted[start] ~ /^#[ \t]+/)
      if (strip_h1) {
