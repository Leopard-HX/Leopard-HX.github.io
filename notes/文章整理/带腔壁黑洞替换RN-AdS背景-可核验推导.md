...
Schwarzschild 腔的离壳热势呈双阱结构：一个阱在 $x=0$，即热平直空间（辐射边界），它是边界局部极小（半阱）、不是驻点，斜率 $\partial_{r_h}F_R|_{r_h=0}=1/(2G)>0$；另一个阱是大支的内部势阱，两阱之间的内部势垒为 $x_s$ 处的小支（$C_V<0$）。因此不存在 2211 中 $x_1,x_2,x_3$ 对应的“两黑洞势阱+中间势垒”结构（这里的第二个阱在边界上），不能直接使用 $r_{k1},r_{k2}$ 的双向解释。
